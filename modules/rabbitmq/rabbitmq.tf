...
resource "datadog_monitor" "high_message_count"{
    name               = "(RMQ) ${var.rabbit_prod_pods} High Message Count"
    type               = "query alert"
    ...
    tags = [
        "rmq:${var.rabbit_prod_pods}",
        "managed_by:terraform"
    ]
}

resource "datadog_monitor" "rabbitmq_memory_usage_is_high"{
    name                = "(RMQ) ${var.rabbit_prod_pods} Memory Usage is High"
    type                = "query alert"
...
resource "datadog_monitor" "node_is_down" {
    name                  = "(RMQ) ${var.rabbit_prod_pods} Node is down"
    type                  = "query alert"
...
resource "datadog_monitor" "rabbitmq_disk_usage_high" {
    name = "(RMQ) ${var.rabbit_prod_pods} Disk Usage is High"
    type = "query alert"
...
resource "datadog_monitor" "rabbitmq_messages_unacknowleged" {
    name = "(RMQ) ${var.rabbit_prod_pods} Unacknowledged Message Rate is High"
    type = "query alert"
